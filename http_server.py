...
接口：
    POST /api/run  提交任务描述 {scenario: str}，同步阻塞执行，返回动作脚本 + 异常应对数据
    GET  /api/health  健康检查
"""
import sys
import json
...
async def _run_task_auto(graph, redis_mgr, scenario: str) -> dict:
    """无确认自动执行：创建会话 → 注入场景 → 图跑完 → 返回动作脚本 + 异常应对数据。"""
    ...
    for _ in range(MAX_DRIVE_ROUNDS):
        ...
        sys.stderr.write(f"[HTTP] 任务未在 {MAX_DRIVE_ROUNDS} 轮内完成\n")
        sys.stderr.flush()

    sys.stderr.write(f"[HTTP] 原始 session_id={session_id}\n")
    sys.stderr.flush()

    for _ in range(MAX_DRIVE_ROUNDS):
        state = await graph.ainvoke(state, config={"recursion_limit": RECURSION_LIMIT})
        sys.stderr.write(f"[HTTP] ainvoke 后 session_id={state.get('session_id')} pending={state.get('pending_question','')[:40]}\n")
        sys.stderr.flush()
        if state.get("session_id") != session_id:
            break
        if state.get("pending_question"):
    ...
    actions_path = script_path(session_id)
    contingency_path = contingency_plan_path(session_id)
    sys.stderr.write(f"[HTTP] 读取 actions: {actions_path} exists={actions_path.is_file()}\n")
    sys.stderr.write(f"[HTTP] 读取 contingency: {contingency_path} exists={contingency_path.is_file()}\n")
    sys.stderr.flush()

    actions_data = {"steps": []}
    if actions_path.is_file():
        try:
            raw = actions_path.read_text(encoding="utf-8")
            actions_data = json.loads(raw)
            sys.stderr.write(f"[HTTP] actions steps count={len(actions_data.get('steps',[]))}\n")
            sys.stderr.flush()
        except Exception as e:
            sys.stderr.write(f"[HTTP] actions parse error: {e}\n")
            sys.stderr.flush()

    contingency_data = {"plan": []}
    if contingency_path.is_file():
        try:
            raw = contingency_path.read_text(encoding="utf-8")
            contingency_data = json.loads(raw)
            sys.stderr.write(f"[HTTP] contingency plan count={len(contingency_data.get('plan',[]))}\n")
            sys.stderr.flush()
        except Exception as e:
            sys.stderr.write(f"[HTTP] contingency parse error: {e}\n")
            sys.stderr.flush()

    return {"actions": actions_data, "contingency": contingency_data}
...
async def run_task(req: RunRequest):
    ...
    result = await _run_task_auto(app.state.graph, app.state.redis_mgr, scenario)
    return result